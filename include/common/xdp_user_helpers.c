											...
											char *sec_name, __u32 xdp_flags, bool reuse_maps)
{
	...
	struct bpf_object *bpf_obj;
	struct xdp_config *xdp_cfg = calloc(1, sizeof(struct xdp_config));
	if(!xdp_cfg)
		printf("[U_XDP_HELPERS]: calloc() failed.\n");
	int offload_ifindex = 0;
	int prog_fd = -1;
	int err;

	if (strlen(filename) >= BPF_FILE_SIZE  && strlen(sec_name) >= BPF_PROGSEC_SIZE
	...
	strcpy(xdp_cfg->progsec, sec_name);
	...
	if (!bpf_obj)
	{
		snprintf(xdp_cfg->err_buf, ERR_BUFF_SIZE,
			"[U_XDP_HELPERS]: load_xdp_object_file_x() failed: %s\n", xdp_cfg->filename);
		return NULL;
	}
	...
	if (prog_fd <= 0)
	{
		snprintf(xdp_cfg->err_buf, ERR_BUFF_SIZE, "ERR: bpf_program__fd failed\n");
		return NULL;
	}
	...
	if (err) {
		snprintf(xdp_cfg->err_buf, ERR_BUFF_SIZE, "ERR: xdp_link_attach failed\n");
		return NULL;
	}
