 ...
 *  - Up-to-date with kernel v4.14-rc8
 *
 * Added features:
...
int attach_cgroup(int prog_fd, int cgroup_fd, enum bpf_attach_type type,
		    unsigned int flags)
{
	return bpf_prog_attach(prog_fd, cgroup_fd, type, flags);
}

int detach_cgroup(int prog_fd, int cgroup_fd, enum bpf_attach_type type)
{
